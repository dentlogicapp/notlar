    ...
    <main className="min-h-screen pb-24">
      ...
      <header className="sticky top-0 z-30 bg-cream-100/85 backdrop-blur-md border-b border-cream-300/60">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between gap-3">
          <Link href="/admin" className="flex items-center gap-1 text-clay-600 hover:text-clay-900 transition-colors min-w-0">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6 sm:py-10 space-y-6">
        <div className="flex items-center gap-3">
          ...
          <h1 className="font-display text-3xl text-clay-900">Denetim Günlüğü</h1>
          <span className="text-sm text-clay-400">({toplam} kayıt)</span>
        </div>
          ...
          <>
            <div className="kart overflow-hidden">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {(data?.kayitlar ?? []).map((d) => (
                    <tr key={d.id} className="border-t border-cream-300 hover:bg-cream-100/40">
                      <td className="py-2.5 px-4 text-xs text-clay-500 whitespace-nowrap">
                        {tarihFormat(d.zaman)}
                      </td>
                      <td className="py-2.5 px-4">
                        <span className="text-xs font-mono text-clay-800 bg-cream-200 px-2 py-0.5 rounded">
                          {d.olay}
                        </span>
                      </td>
                      <td className="py-2.5 px-4 text-clay-700 hidden md:table-cell text-xs">
                        {d.aktorEmail ?? "—"}
                      </td>
                      <td className="py-2.5 px-4 text-xs text-clay-500 hidden lg:table-cell font-mono">
                        {d.ip ?? "—"}
                      </td>
                      <td className="py-2.5 px-4 text-xs text-clay-600 hidden md:table-cell max-w-xs truncate" title={d.detay ?? ""}>
                        {d.detay ?? "—"}
                      </td>
                    </tr>
                  ))}
            ...
            {sayfaSayisi > 1 && (
              <div className="flex items-center justify-between text-sm">
                <span className="text-clay-500">{sayfa} / {sayfaSayisi}</span>
